...
export const generateMetadata = async ({
  ...
  return generateMetadataUtil({
    title: getSeo(locale).pages.contact.title,
    description: getSeo(locale).pages.contact.description,
...
const ContactPage = async ({ params }: { params: Promise<{ locale: string }> }) => {
  const locale = await localeFromParams(params);
  const { title, description } = getSeo(locale).pages.contact;
      ...
      <div className="container mx-auto grid gap-6 px-4 pt-10 md:px-6 md:pt-14 lg:grid-cols-5">
        <Card className="h-fit border-border/70 lg:col-span-2">
          <CardContent className="space-y-5 p-6 md:p-8">
            <h2 className="text-heading-3">{t('contactDetails')}</h2>
            <ul className="space-y-4 text-body-sm text-secondary">
              ...
              <li className="flex items-start gap-3">
                <MapPin
                  ...
                  aria-hidden="true"
                />
                <span>{t('contactFlagship')}</span>
              </li>
            </ul>
            <div aria-hidden="true" className="rule-gradient" />
            <p className="text-caption text-secondary">{t('contactOrdersHint')}</p>
          </CardContent>
        </Card>
        <Card className="border-border/70 lg:col-span-3">
          <CardContent className="p-6 md:p-8">
            <h2 className="text-heading-3">{t('contactFormTitle')}</h2>
            <p className="text-body-sm mt-1 text-secondary">{t('contactFormSubtitle')}</p>
