  ...
  return (
    <Card>
      <CardHeaderPattern
        ...
        description={t('deliveryEstimateHint')}
      />
      <CardContent className="space-y-4" aria-busy={isBusy}>
        {showForm ? (
          <div className="space-y-3">
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="space-y-2">
                <Label htmlFor="delivery-postal">{t('postalCode')}</Label>
                <Input
                  id="delivery-postal"
                  value={postal}
        ...
        {groups.map((group) => (
          <fieldset key={group.id} className="space-y-2">
            <legend className="text-body-sm font-medium">{t('deliveryMethod')}</legend>
              ...
              return (
                ...
                >
                  ...
                  <span className="flex-1">
                    <span className="block font-medium">{option.title ?? t('shipping')}</span>
                    {option.description ? (
        ...
        {!showForm && groups.length === 0 ? (
          <p className="text-caption text-secondary">{t('noDeliveryOptions')}</p>
        ) : null}
      </CardContent>
    </Card>
  );
