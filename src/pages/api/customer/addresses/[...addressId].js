...
export default async function handler(req, res) {
  try {
    ...
    if (!addressId) return res.status(400).json({ message: 'Missing addressId param' });

    const id = decodeURIComponent(addressId);

    switch (method) {
      case 'GET': {
        const responseAddress = await getCustomerAddressById(token, id, delegateToken, ip);
      ...
      case 'PUT': {
        ...
        if (!address) return res.status(400).send({ message: 'Missing query parameter' });
        const updatedAddress = await updateAddress(address, token, id, delegateToken, ip);
